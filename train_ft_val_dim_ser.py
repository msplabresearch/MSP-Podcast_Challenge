...
for dtype in ["train", "dev"]:
    cur_utts, cur_labs = utils.load_adv_valence(label_path, dtype)
...
lm = utils.LogManager()
lm.alloc_stat_type_list(["train_val"])
lm.alloc_stat_type_list(["dev_val"])
...
for epoch in range(EPOCHS):
    ...
    for xy_pair in tqdm(total_dataloader["train"]):
        ...
        # Logging
        lm.add_torch_stat("train_val", ccc[0])
    ...
    # Logging
    lm.add_torch_stat("dev_val", ccc[0])

    # Save model
    lm.print_stat()


    dev_loss = 1.0 - lm.get_stat("dev_val")
